    ...
    public func handle(action: WalletActions, store: Store<ApplicationState>, handler: @escaping (AnyAction?) -> Void) {
        ...
        walletQueue.async {
            do {
                ...
                if let lokiWallet = currentWallet as? LokiWallet {
                    pendingTransaction = try lokiWallet.send(
                        amount: amount,
                        to: address,
                        paymentID: paymentID,
...
//        workQueue.async {
            do {
                ...
                let lokiWallet = currentWallet as! LokiWallet
                ...
                try lokiWallet.rescan(from: height, password: password)
...
//                guard let lokiWallet = currentWallet as? LokiWallet else {
//                    return
//                }
//
//                try lokiWallet.rescan(
//                    from: height,
//                    password: try KeychainStorageImpl.standart.fetch(forKey: KeychainKey.walletPassword(WalletIndex(name: lokiWallet.name, type: .loki))),
//                    node: node)
//                handler(
        ...
        walletQueue.async {
            do {
                ...
                if error.localizedDescription == "std::bad_alloc" {
                    try! LokiWalletGateway.init().removeCacheFile(for: name)
...
// fixme
private func restoreFromMyloki(seed: String, name: String, restoreHeight: UInt64, handler: @escaping (AnyAction?) -> Void, completionHandler: @escaping () -> Void) {
    let mn = mndecode(seed: seed)
    var _d = toByteArray(mn)
    let psk = LokiWalletAdapter.psk(&_d)!
    let pvk = LokiWalletAdapter.pvk(&_d)!
    let pubSk = LokiWalletAdapter.secretKey(toPublic: psk)!
    let pubVk = LokiWalletAdapter.secretKey(toPublic: pvk)!
    
    let address = LokiWalletAdapter.getAddressFromViewKey(pubVk, andSpendKey: pubSk)!
...
public struct RestoreFromSeedWalletHandler: AsyncHandler {
    public func handle(action: WalletActions, store: Store<ApplicationState>, handler: @escaping (AnyAction?) -> Void) {
        ...
        if words == 13 {
            restoreFromMyloki(seed: seed, name: name, restoreHeight: restoreHeight, handler: handler, completionHandler: completionHandler)
            return
        }
