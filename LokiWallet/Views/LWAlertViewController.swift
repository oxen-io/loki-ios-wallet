...
final class LWAlertViewController: BaseViewController<LWAlertView> {
    enum Status {
    ...
    func addActions(_ actions: [LWAlertAction], axis: UILayoutConstraintAxis? = nil) {
        ...
        for (index, action) in sortedActions.enumerated() {
            ...
            //            action.frame = CGRect(origin: .zero, size: CGSize(width: width, height: LWAlertAction.defaultHeight))
        ...
        let actionsStackViewHeight = actionsAxis == .vertical ? LWAlertAction.defaultHeight * CGFloat(sortedActions.count) : LWAlertAction.defaultHeight
