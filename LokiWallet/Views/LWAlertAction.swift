...
final class LWAlertAction: BaseView {
    enum Style {
        case `default`, cancel
    }
    ...
    static var okAction: LWAlertAction {
        return LWAlertAction(
            title: NSLocalizedString("ok", comment: ""),
            handler: { $0.alertView?.dismiss(animated: true) }
        )
    }
    
    static var cancelAction: LWAlertAction {
        return LWAlertAction(
    ...
    let title: String
    let handler: (LWAlertAction) -> Void
    let style: Style
    weak var alertView: LWAlertViewController?
    
    init(title: String, style: Style = .default, handler: @escaping (LWAlertAction) -> Void) {
        self.style = style
    ...
    override func configureView() {
        ...
        titleLabel.textColor = .lokiGreen
