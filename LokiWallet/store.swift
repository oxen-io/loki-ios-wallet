...
private func getSavedNode() -> NodeDescription? {
    ...
    let login = UserDefaults.standard.string(forKey: Configurations.DefaultsKeys.nodeLogin),
        let password = UserDefaults.standard.string(forKey: Configurations.DefaultsKeys.nodePassword) else {
            return Configurations.defaultLokiNode
    }
    
    return LokiNodeDescription(uri: uri, login:  login, password:  password)
}
...
let store = Store<ApplicationState>(
    initialState: ApplicationState(
        walletState: WalletState(
            ...
            balance: LokiAmount(value: 0),
            unlockedBalance: LokiAmount(value: 0),
            price: 0,
            ...
            transactions: [],
            sendingStage: .none,
            estimatedFee: LokiAmount(value: 0)
