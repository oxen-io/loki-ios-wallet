...
struct LokiQRResult: QRUri {
    ...
    var amount: Amount? {
        ...
        return LokiAmount(from: amountStr)
    }
...
//        }
    ...
    func update(uri: QRUri) {
        guard let uri = uri as? LokiQRResult else {
            return
        }
    ...
    private func updateEstimatedFee(_ fee: Amount) {
        let formattedFee = LokiAmountParser.formatValue(fee.value) ?? "0.0"
    ...
    private func onTransactionCreating() {
        let sendAction = LWAlertAction(title: NSLocalizedString("send", comment: "")) { [weak self] action in
        ...
        showInfo(
            ...
            actions: [sendAction, LWAlertAction.cancelAction]) { alert in
    ...
    private func onTransactionCreated(_ pendingTransaction: PendingTransaction) {
        ...
        let message = NSLocalizedString("commit_transaction", comment: "")
            ...
            + ": "
            + LokiAmountParser.formatValue(description.fee.value)
        let commitAction = LWAlertAction(title: NSLocalizedString("Ok", comment: "")) { [weak self] action in
            action.alertView?.dismiss(animated: true) {
                self?.commit(pendingTransaction: pendingTransaction)
            }
        }
        let cacelAction = LWAlertAction(title: NSLocalizedString("cancel", comment: ""), style: .cancel) { [weak self] action in
            action.alertView?.dismiss(animated: true) {
                self?.store.dispatch(
    ...
    private func onTransactionCommited() {
        let okAction = LWAlertAction(title: NSLocalizedString("Ok", comment: "")) { [weak self] action in
            action.alertView?.dismiss(animated: true) {
                self?.resetForm()
            }
        }
            ...
            authController.dismiss(animated: true) {
                self?.showSpinner(withTitle: NSLocalizedString("creating_transaction", comment: ""), callback: { alert in
                    let amount = self?.contentView.cryptoAmountTextField.text == SendViewController.allSymbol
                        ? nil
                        : LokiAmount(from: self!.contentView.cryptoAmountTextField.text?.replacingOccurrences(of: ",", with: "") ?? "0.0")
