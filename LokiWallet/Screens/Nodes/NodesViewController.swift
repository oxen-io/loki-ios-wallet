...
let nodesQueue = DispatchQueue(label: "app.lokiwallet.nodes-queue", qos: .default, attributes: DispatchQueue.Attributes.concurrent)
...
final class NodesViewController: BaseViewController<NodesView>, UITableViewDelegate, UITableViewDataSource, StoreSubscriber {
    ...
    func askToChangeCurrentNode(to node: NodeDescription) {
        let changeAction = LWAlertAction(title: NSLocalizedString("change", comment: "")) { [weak self] alert in
            self?.changeCurrentNode(to: node)
            alert.alertView?.dismiss(animated: true)
        }
        showInfo(title: String(format: NSLocalizedString("change_current_node_message", comment: ""), node.uri), actions: [changeAction, LWAlertAction.cancelAction])
    }
    ...
    private func resetNodesList() {
        let resetAction = LWAlertAction(title: NSLocalizedString("reset", comment: "")) { [weak self] action in
            do {
                try NodesList.shared.reset()
                self?.updateNodes()
                self?.changeCurrentNode(to: Configurations.defaultLokiNode)
        ...
        showInfo(title: NSLocalizedString("node_reset_settings_title", comment: ""), message: NSLocalizedString("nodes_list_reset_to_default_message", comment: ""), actions: [resetAction, LWAlertAction.cancelAction])
    }
    //fixme
    private func removeNode(at indexPath: IndexPath) {
        let removeAction = LWAlertAction(title: NSLocalizedString("remove", comment: "")) { [weak self] _ in
            do {
        ...
        showInfo(
            ...
            actions: [removeAction, LWAlertAction.cancelAction]
        )
    }
}
