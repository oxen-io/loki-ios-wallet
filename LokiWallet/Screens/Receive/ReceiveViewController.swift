...
final class ReceiveViewController: BaseViewController<ReceiveView> {
    ...
    var amount: Amount? {
        get {
            if let rawString = contentView.amountTextField.text {
                return LokiAmount(from: rawString)
            }
    ...
    private func generatePaymentId() {
        // FIX-ME: We don't need know anout LokiWalletAdapter, but it's should not be part of WalletProtocol.
        guard
            let paymentId = LokiWalletAdapter.generatePaymentId(),
            let wallet = currentWallet as? LokiWallet else {
            return
        }
