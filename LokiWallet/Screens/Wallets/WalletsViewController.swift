                        ...
                        handler: {
                            alert.dismiss(animated: true) {
                                if  let error = self?.store.state.error {
                                    self?.showInfo(title: nil, message: error.localizedDescription, actions: [LWAlertAction.cancelAction])
                                    return
                                }
...
//            authController?.present(UINavigationController(rootViewController: showKeysViewController), animated: true)
    ...
    private func askToRemove(wallet: WalletIndex) {
        let removeAction = LWAlertAction(title: NSLocalizedString("remove", comment: "")) { alert in
