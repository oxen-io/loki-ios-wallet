...
private let lokiUserAgent = "LokiWallet/XMR iOS"
...
final class ExchangeActionCreators {
    ...
    private static let ref = "lokiwallet"
    ...
    func createTradeXMRTO(amount: Amount, address: String, handler: @escaping (LokiWalletLib.Result<String>) -> Void) {
            let url =  URLComponents(string: String(format: "%@/order_create/", xmrtoUri))!
            var request = URLRequest(url: url.url!)
            request.httpMethod = "POST"
            request.addValue("application/json", forHTTPHeaderField: "Content-Type")
            request.addValue(lokiUserAgent, forHTTPHeaderField: "User-Agent")
            let bodyJSON: JSON = [
    ...
    func getOrderStatusForXMRTO(uuid: String) -> Store<ApplicationState>.AsyncActionProducer {
        return { state, store, handler in
            ...
            Alamofire.request(request).responseData(completionHandler: { response in
                ...
                let amount = LokiAmount(from: totalAmount)
                ...
                let trade = ExchangeTrade(
                    ...
                    min: LokiAmount(value: 0),
                    max: LokiAmount(value: 0),
                    value: amount,
                    status: state,
                    paymentId: paymentId,
                    provider: .xmrto,
    ...
    func createTrade(from input: CryptoCurrency, refund: String, outputs: [ExchangeOutput], handler: @escaping (LokiWalletLib.Result<ExchangeTrade>) -> Void) {
//        return { state, store, handler in
            DispatchQueue.global(qos: .utility).async {
                ...
                Alamofire.request(request).responseData(completionHandler: { response in
                    ...
                    case .loki:
                        min = LokiAmount(value: UInt64(minAmount))
                        max = LokiAmount(value: UInt64(maxAmount))
...
//        case .loki:
//            return LokiAmount(from: stringAmount)
//        }
//    }
//
//    var refund: String {
...
private func fetchLimits(for inputAsset: CryptoCurrency, and outputAsset: CryptoCurrency, outputWeight: Int = 10000, handler: @escaping (LokiWalletLib.Result<Limits>) -> Void) {
    DispatchQueue.global(qos: .utility).async {
...
private func fetchXMRTOLimits(handler: @escaping (LokiWalletLib.Result<(min: Double, max: Double)>) -> Void) {
    DispatchQueue.global(qos: .utility).async {
