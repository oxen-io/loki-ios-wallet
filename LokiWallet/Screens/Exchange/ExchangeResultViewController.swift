...
//    }
    ...
    func updateQR(address: String, paymentID: String? = nil, amount: Amount? = nil) {
        ...
        if
            ...
            let uri = LokiUri(address: address, paymentId: paymentID, amount: amount)
            text = uri.formatted()
        } else {
            text = address
        }
...
//        updateMaxAmount(with: trade.max)
    ...
    private func onTransactionCreating() {
        ...
        showInfo(
            ...
            actions: [sendAction, LWAlertAction.cancelAction]
        )
    }
    ...
    private func onTransactionCreated(_ pendingTransaction: PendingTransaction) {
        ...
        let cancelAction = LWAlertAction(title: NSLocalizedString("cancel", comment: ""), style: .cancel) { [weak self] action in
            action.alertView?.dismiss(animated: true) {
                self?.store.dispatch(
    ...
    private func commit(pendingTransaction: PendingTransaction) {
        ...
        if
            let lokiPendingTransaction = pendingTransaction as? LokiPendingTransaction,
            let _transactionID = lokiPendingTransaction.id {
