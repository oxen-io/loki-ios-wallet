...
final class NewNodeViewController: BaseViewController<NewNodeView> {
    ...
    private func onResetSetting() {
        setSettings(LokiNodeDescription(uri: "", login: "", password: ""))
    }
    ...
    private func save() {
        ...
        showSpinner(withTitle: NSLocalizedString("saving", comment: ""), callback: { alert in
            ...
            let nodeDescription = LokiNodeDescription(
                uri: uri,
            ...
            do {
                ...
                alert.dismiss(animated: true) { [weak self] in
                    let okAction = LWAlertAction(title: NSLocalizedString("ok", comment: ""), handler: { action in
