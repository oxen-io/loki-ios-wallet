...
//    }
    ...
    private func reconnectAction() {
        let reconnetionAction = LWAlertAction(title: NSLocalizedString("reconnect", comment: "")) { [weak self] action in
            self?.store.dispatch(WalletActions.reconnect)
            action.alertView?.dismiss(animated: true)
        }
        showInfo(title: NSLocalizedString("reconnection", comment: ""), message: NSLocalizedString("reconnect_alert_text", comment: ""), actions: [reconnetionAction, LWAlertAction.cancelAction])
    }
