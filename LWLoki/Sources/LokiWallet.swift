import Foundation
import LokiWalletLib
import Alamofire

// fixme: remove and rework this shit

private let lokiBlockSize = 1000
public let walletQueue = DispatchQueue(label: "app.lokiwallet.loki-wallet-queue", qos: .default)

public final class CachedValue<Val> {
    public var handler: (() -> Val)?
    public var filter: ((Val) -> Bool)?
    private var origin: Val
    private var lastUpdateDate: Date
    private let timeout: TimeInterval
    
    public init(origin: Val, timeout: TimeInterval, handler: (() -> Val)? = nil) {
        self.origin = origin
        self.timeout = timeout
        self.handler = handler
        self.lastUpdateDate = Date(timeIntervalSince1970: 0)
    }
    
    public func value() -> Val {
        
        if
            let filter = filter,
            filter(origin),
            let newVal = handler?() {
            origin = newVal
            lastUpdateDate = Date()
            return origin
        }
        
        let diff = Date().timeIntervalSince(lastUpdateDate)
        
        if
            diff > timeout,
            let newVal = handler?() {
            origin = newVal
            lastUpdateDate = Date()
        }
        
        return origin
    }
    
    public func isInvalided() -> Bool {
        let diff = Date().timeIntervalSince(lastUpdateDate)
        let isInvalided = diff > timeout
        return isInvalided
    }
}

private let cachedBlockchainTimeout: TimeInterval = 30
private var cachedBlockchainHeight = CachedValue(origin: UInt64(0), timeout: cachedBlockchainTimeout)

public final class LokiWallet: Wallet {
    public static let walletType = WalletType.loki
    
    public var name: String {
        return lokiAdapter.name()
    }
    
    public var balance: Amount {
        return LokiAmount(value: lokiAdapter.balance())
    }
    
    public var unlockedBalance: Amount {
        return LokiAmount(value: lokiAdapter.unlockedBalance())
    }
    
    public var address: String {
        return lokiAdapter.address()
    }
    
    public var seed: String {
        return lokiAdapter.seed()
    }
    
    public var isConnected: Bool {
        return lokiAdapter.connectionStatus() != 0
    }
    
    public var keys: WalletKeys {
        return _keys
    }
    
    public var isWatchOnly: Bool {
        return _keys.spendKey.sec.range(of: "^0*$", options: .regularExpression, range: nil, locale: nil) != nil
    }
    
    public var currentHeight: UInt64 {
        let height = lokiAdapter.currentHeight()
        
        if config.isRecovery && (height == 1 || height == 0) {
            return restoreHeight
        } else {
            return height
        }
    }
    
    public var onNewBlock: ((UInt64) -> Void)?
    public var onBalanceChange: ((Wallet) -> Void)?
    public var onConnectionStatusChange: ((ConnectionStatus) -> Void)?
    public private(set) var config: WalletConfig
    private var isBlocking: Bool
    
    private var lokiTransactionHistory: LokiTransactionHistory?
    
    private var _keys: LokiWalletKeys {
        return LokiWalletKeys(
            spendKey: LokiWalletKeysPair(pub: self.lokiAdapter.publicSpendKey(), sec: self.lokiAdapter.secretSpendKey()),
            viewKey: LokiWalletKeysPair(pub: self.lokiAdapter.publicViewKey(), sec: self.lokiAdapter.secretViewKey()))
    }
    
    private var lokiAdapter: LokiWalletAdapter
    private var restoreHeight: UInt64
    
    public convenience init(lokiAdapter: LokiWalletAdapter, config: WalletConfig, restoreHeight: UInt64) {
        self.init(lokiAdapter: lokiAdapter, config: config)
        self.restoreHeight = restoreHeight
//        self.lokiAdapter.setRefreshFromBlockHeight(restoreHeight)
    }
    
    public init(lokiAdapter: LokiWalletAdapter, config: WalletConfig) {
        self.lokiAdapter = lokiAdapter
        self.isBlocking = false
        self.config = config
        restoreHeight = 0
        self.lokiAdapter.delegate = self
        lokiTransactionHistory = nil
        
        
        if config.isRecovery {
            lokiAdapter.setIsRecovery(config.isRecovery)
        }
    }
    
    deinit {
        print("LokiWallet deinit")
    }
    
    public func blockchainHeight() throws -> UInt64 {
        cachedBlockchainHeight.filter = { val in
            return val == 0
        }
        
        cachedBlockchainHeight.handler = { [weak self] in
            return self?.lokiAdapter.daemonBlockChainHeight() ?? 0
        }
        
        return cachedBlockchainHeight.value()
    }
    
    public func changePassword(newPassword: String) throws {
        try lokiAdapter.setPassword(newPassword)
    }
    
    public func save() throws {
        guard !isBlocking else {
            return
        }
        
        try walletQueue.sync {
            print("saving: \(Date())")
            try lokiAdapter.save()
        }
    }
    
    public func connect(toNode node: NodeDescription) throws {
        guard !isBlocking else {
            return
        }
        
        lokiAdapter.setDaemonAddress(node.uri, login: node.login, password: node.password)
        try lokiAdapter.connectToDaemon()
    }
    
    public func close() {
        isBlocking = true
        lokiAdapter.delegate = nil
        
        walletQueue.sync {
            print("closing: \(Date())")
            lokiAdapter.close()
            lokiAdapter.clear()
        }
    }
    
    public func startUpdate() {
        guard !isBlocking else {
            return
        }
        
        lokiAdapter.startRefreshAsync()
    }  
    
    public func transactions() -> TransactionHistory {
        if let lokiTransactionHistory = self.lokiTransactionHistory {
            return lokiTransactionHistory
        } else {
            let _lokiTransactionHistory = LokiTransactionHistory(lokiWalletHistoryAdapter: LokiWalletHistoryAdapter(wallet: lokiAdapter))
            self.lokiTransactionHistory = _lokiTransactionHistory
            return _lokiTransactionHistory
        }
    }
    
    public func subaddresses() -> Subaddresses {
        return Subaddresses(wallet: lokiAdapter)
    }
    
    public func send(amount: Amount?, to address: String, withPriority priority: TransactionPriority) throws -> PendingTransaction {
        do {
            let lokiPendingTransactionAdapter = try self.lokiAdapter.createTransaction(
                toAddress: address,
                withPaymentId: "",
                amountStr: amount?.formatted(),
                priority: priority.rawValue)
            return LokiPendingTransaction(lokiPendingTransactionAdapter: lokiPendingTransactionAdapter)
        } catch let error as NSError {
            if let transactionError = TransactionError(from: error, amount: amount, balance: self.balance) {
                throw transactionError
            } else {
                throw error
            }
        }
    }
    
    public func send(amount: Amount?, to address: String, paymentID: String = "", withPriority priority: TransactionPriority) throws -> PendingTransaction {
        do {
            let lokiPendingTransactionAdapter = try self.lokiAdapter.createTransaction(
                toAddress: address,
                withPaymentId: paymentID,
                amountStr: amount?.formatted(),
                priority: priority.rawValue)
            return LokiPendingTransaction(lokiPendingTransactionAdapter: lokiPendingTransactionAdapter)
        } catch let error as NSError {
            if let transactionError = TransactionError(from: error, amount: amount, balance: self.balance) {
                throw transactionError
            } else {
                throw error
            }
        }
    }
    
    public func integratedAddress(for paymentId: String) -> String {
        return self.lokiAdapter.integratedAddress(for: paymentId)
    }
    
    public func rescan(from height: UInt64, node: NodeDescription) throws {
//        let isRecovery = true
//        LokiWalletGateway().recoveryWallet(withName: name, andSeed: seed, password: "", restoreHeight: 0)
//        lokiAdapter.pauseRefresh()
//        try LokiWalletGateway().removeCacheFile(for: name)
//        lokiAdapter.setIsRecovery(isRecovery)
//        lokiAdapter.setRefreshFromBlockHeight(height)
//        try config.update(isRecovery: isRecovery)
//        try lokiAdapter.save()
//        try connect(toNode: node)
//        startUpdate()
    }
    
    public func rescan(from height: UInt64, password: String) throws {
        try walletQueue.sync {
            isBlocking = true
            print("rescan start")
            let _name = self.lokiAdapter.name()!
            let _seed = self.lokiAdapter.seed()!
            let gateway = LokiWalletGateway()
            self.lokiAdapter.delegate = nil
            self.lokiAdapter.close()
            self.lokiAdapter.clear()
            try gateway.remove(withName: _name)
            let _lokiAdapter = LokiWalletAdapter()!
            try _lokiAdapter.recovery(at: gateway.makeURL(for: _name).path, mnemonic: _seed, andPassword: password, restoreHeight: height)
            self.lokiAdapter = _lokiAdapter
            self.lokiTransactionHistory = nil
            let walletConfig = WalletConfig(isRecovery: true, date: Date(), url: gateway.makeConfigURL(for: _name))
            try walletConfig.save()
            self.config = walletConfig
            print("rescan end")
            isBlocking = false
            self.lokiAdapter.delegate = self
        }
    }
}

// MARK: LokiWallet + LokiWalletAdapterDelegate

extension LokiWallet: LokiWalletAdapterDelegate {
    public func newBlock(_ block: UInt64) {
        onNewBlock?(block)
    }

    public func updated() {
        print("updated")
    }

    public func refreshed() {
//        updateQueue.async {
            do {
//                let currentHeight = currentHeight
                let blockchainheight = try blockchainHeight()
                let isRecovery = config.isRecovery
                
                guard currentWallet.currentHeight != 0 && blockchainheight != 0 else {
                    return
                }
                
                if (blockchainheight >= currentHeight && blockchainheight - currentHeight < lokiBlockSize) {
                    if currentHeight == blockchainheight && isRecovery {
                        try currentWallet.config.update(isRecovery: false)
                        self.onConnectionStatusChange?(.synced)
                        return
                    }
                    
                    if !isRecovery {
                        self.onConnectionStatusChange?(.synced)
                    }
                }
                
                if isRecovery {
                    try save()
                }
            } catch {
                print(error)
            }
    }

    public func moneyReceived(_ txId: String!, amount: UInt64) {
        onBalanceChange?(self)
    }

    public func moneySpent(_ txId: String!, amount: UInt64) {
        onBalanceChange?(self)
    }

    public func unconfirmedMoneyReceived(_ txId: String!, amount: UInt64) {
        onBalanceChange?(self)
    }
}
