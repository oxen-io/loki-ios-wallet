...
#import "LokiWalletError.h"
#import "LokiPendingTransactionAdapter.mm"
...
struct MonerWalletListener: Loki::WalletListener {
    LokiWalletAdapter *wallet;
...
struct LokiWalletAdapterMember {
public:
    Loki::WalletImpl *wallet;
    MonerWalletListener *listener;
};

@implementation LokiWalletAdapter: NSObject

+ (NSString *) generatePaymentId
{
    NSString *paymentId = [NSString stringWithUTF8String: Loki::Wallet::genPaymentId().c_str()];
    return paymentId;
}
...
- (id)init
{
    ...
    if (self) {
        ...
        Loki::NetworkType nettype = Loki::NetworkType::MAINNET;
#ifdef USE_TESTNET
        NSLog(@"TESTNET ENABLED");
        nettype = Loki::NetworkType::TESTNET;
#endif
        
        Loki::WalletImpl *wallet = new Loki::WalletImpl(nettype);
        MonerWalletListener *listener = new MonerWalletListener();
        listener->wallet = self;
        wallet->setListener(listener);
        member = new LokiWalletAdapterMember();
        member->wallet = wallet;
        member->listener = listener;
        Loki::WalletManagerFactory::setLogLevel(1);
    }
...
- (BOOL)lightWalletLogin: (BOOL) isNewWallet error:(NSError **) error
{
    ...
    //            *error = [NSError errorWithDomain: LokiWalletErrorDomain
    //                                         code: LokiWalletCreatingError
...
- (uint64_t)connectionStatus
{
    try {
        ...
        {
            Loki::Wallet::ConnectionStatus status = member->wallet->connected();
...
- (BOOL)recoveryAt:(NSString *)path mnemonic: (NSString *)seed andPassword:(NSString *) password restoreHeight: (uint64_t) restoreHeight error:(NSError **) error
{
    ...
    if (!isRecovered) {
        if (error != NULL) {
            NSString* errorDescription = [NSString stringWithUTF8String: member->wallet->errorString().c_str()];
            *error = [NSError errorWithDomain: LokiWalletErrorDomain
                                         code: LokiWalletRecoveringError
                                     userInfo: @{ NSLocalizedDescriptionKey: errorDescription }];
        }
...
- (LokiPendingTransactionAdapter *)createTransactionToAddress: (NSString *) address WithPaymentId: (NSString *) paymentId amountStr: (NSString *) amount_str priority: (UInt64) priority error: (NSError *__autoreleasing *) error
{
    ...
    Loki::PendingTransaction::Priority _priopity = static_cast<Loki::PendingTransaction::Priority>(priority);
    Loki::PendingTransaction *tx;
    if (amount_str != nil) {
        uint64_t amount;
        ...
        tx = member-> wallet->createTransaction(addressStdString, paymentIdStdString, Loki::optional<uint64_t>(), mixin, _priopity);
    }
    
    int status = tx->status();
    
    if (status == Loki::PendingTransaction::Status::Status_Error
        || status == Loki::PendingTransaction::Status::Status_Critical) {
        
        if (error != NULL) {
            NSString* errorDescription = [NSString stringWithUTF8String: tx->errorString().c_str()];
            *error = [NSError errorWithDomain: LokiWalletErrorDomain
                                         code: LokiWalletTransactionCreatingError
                                     userInfo: @{ NSLocalizedDescriptionKey: errorDescription }];
        }
    ...
    LokiPendingTransactionMember *ptxMember = new LokiPendingTransactionMember();
    ptxMember->tx = tx;
    LokiPendingTransactionAdapter *ptx = [[LokiPendingTransactionAdapter alloc] initWithMember: ptxMember];
...
- (BOOL)connectToDaemon: (NSError **) error
{
    ...
    if (!isConnected) {
        if (error != NULL) {
            NSString* errorDescription = [NSString stringWithUTF8String: member->wallet->errorString().c_str()];
            *error = [NSError errorWithDomain: LokiWalletErrorDomain
                                         code: LokiWalletConnectingError
                                     userInfo: @{ NSLocalizedDescriptionKey: errorDescription }];
        }
...
- (Loki::TransactionHistory *)rawHistory
{
...
- (BOOL)setPassword:(NSString *) password error:(NSError **) error
{
    ...
    if (!changed) {
        if (error != NULL) {
            NSString* errorDescription = [NSString stringWithUTF8String: member->wallet->errorString().c_str()];
            *error = [NSError errorWithDomain: LokiWalletErrorDomain
                                         code: LokiWalletPasswordChangingError
                                     userInfo: @{ NSLocalizedDescriptionKey: errorDescription }];
        }
