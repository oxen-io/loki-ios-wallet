...
Loki::SubaddressImpl *subaddressAccount;

- (instancetype)initWithWallet: (LokiWalletAdapter *) wallet
{
    self = [super init];
    if (self) {
        subaddressAccount = new Loki::SubaddressImpl(wallet->member->wallet);
    }
    return self;
}
...
- (NSArray *)getAll
{
    std::vector<Loki::SubaddressRow*> _subs = subaddressAccount->getAll();
    std::size_t count = _subs.capacity();
    NSMutableArray *result = [[NSMutableArray alloc]init];
    
    for (int i = 0; i < count; i++) {
        Loki::SubaddressRow *_sub =_subs[i];
