...
public final class LokiWalletGateway: WalletGateway {
    ...
    public func create(withName name: String, andPassword password: String) throws -> Wallet {
        let lokiAdapter = LokiWalletAdapter()!
        try lokiAdapter.generate(withPath: self.makeURL(for: name).path, andPassword: password)
        ...
        return LokiWallet(lokiAdapter: lokiAdapter, config: walletConfig)
    }
    
    public func load(withName name: String, andPassword password: String) throws -> Wallet {
        let lokiAdapter = LokiWalletAdapter()!
        let path = self.makeURL(for: name).path
        try lokiAdapter.loadWallet(withPath: path, andPassword: password)
    ...
    public func recoveryWallet(withName name: String, andSeed seed: String, password: String, restoreHeight: UInt64) throws -> Wallet {
        let lokiAdapter = LokiWalletAdapter()!
        try lokiAdapter.recovery(at: self.makeURL(for: name).path, mnemonic: seed, andPassword: password, restoreHeight: restoreHeight)
//        try lokiAdapter.setPassword(password)
//        lokiAdapter.setRefreshFromBlockHeight(restoreHeight)
//        lokiAdapter.setIsRecovery(true)
//        try lokiAdapter.save()
        let walletConfig = WalletConfig(isRecovery: true, date: Date(), url: self.makeConfigURL(for: name))
        try walletConfig.save()
        return LokiWallet(lokiAdapter: lokiAdapter, config: walletConfig, restoreHeight: restoreHeight)
    }
    ...
    public func recoveryWallet(withName name: String, publicKey: String, viewKey: String, spendKey: String, password: String, restoreHeight: UInt64) throws -> Wallet {
        let lokiAdapter = LokiWalletAdapter()!
        try lokiAdapter.recoveryFromKey(at: self.makeURL(for: name).path, withPublicKey: publicKey, andPassowrd: password, andViewKey: viewKey, andSpendKey: spendKey, withRestoreHeight: restoreHeight)
//        try lokiAdapter.setPassword(password)
//        lokiAdapter.setRefreshFromBlockHeight(restoreHeight)
//        lokiAdapter.setIsRecovery(true)
//        try lokiAdapter.save()
...
//        workQueue.async {
            ...
            self.fetchFeePerKb(withNode: node) { result in
                ...
                case let .success(feePerKb):
                    ...
                    let fee = LokiAmount(value: feeValue)
